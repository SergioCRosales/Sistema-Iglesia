...
if (!empty($_POST)) {
  ...
  if (empty($_POST['idbautismo'])) {
    ...
    if (is_numeric($idbautismo) and $idbautismo != 0) {
      ...
      $query = mysqli_query($conexion, "SELECT * FROM matrimoniocombm where (idbautismo = '$idbautismo' AND idmatrimonio != $idmatrimonio)");
    ...
    if ($resul >= 1) {
      ...
      $sql_update = mysqli_query($conexion, "UPDATE matrimoniocombm SET  partida = $partida, folio = '$folio', libro = '$libro', fecha = '$fecha', idtestiga = '$idtestiga', idtestigo = '$idtestigo', idbautismo = '$idbautismo', edadesposo = '$edadesposo', idfeligres = '$idfeligres', edadesposa = '$edadesposa', idpadre = '$idpadre', idmadre = '$idmadre', idministro = '$idministro', observaciones = '$observaciones' WHERE idmatrimonio = $idmatrimonio");
...
$sql = mysqli_query($conexion, "SELECT * FROM matrimoniocombm WHERE idmatrimonio = $idmatrimonio");
...
if ($result_sql == 0) {
  ...
  while ($data = mysqli_fetch_array($sql)) {
    ...
    $consultaactual= mysqli_query($conexion,"SELECT * FROM matrimoniocombm WHERE idmatrimonio='$idmatrimonio'");
...
?>
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h4 mb-0 text-gray-800">Editar datos de constancia de Matrimonio (Esposa de esta parroquia)</h1>
       
        <a href="lista_matrimoniomixtom.php" class="btn btn-primary">Ver listado matrimonio</a>
    </div>
