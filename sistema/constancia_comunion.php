...
$pdf->Ln();
///pruebas
$pdf->Image('img/comunion.jpg' , 10 ,10, 30 , 33,'JPG');
...
$pdf->SetFont('Times', 'B', 14);
$pdf->Cell(0, 6, utf8_decode('CONSTANCIA DE PRIMERA COMUNION'), 0, 1,'C');
...
c.observaciones
FROM comunion as c
JOIN bautismo as b on c.idbautismo = b.idbautismo
...
JOIN madrina as mdr on c.idmadrina = mdr.idmadrina
JOIN feligreses as f3 on mdr.idfeligres = f3.idfeligres
JOIN padrino as pdr on c.idpadrino = pdr.idpadrino
...
while ($data = mysqli_fetch_assoc($query)) {
	...
	$pdf->SetFont('Times','B'.'U',12);
	$pdf->SetTextColor(5, 5, 5 );
	$pdf->Write(7,'   '.$data['foliocm'].'   ');
	$pdf->SetFont('Times','',12);
	$pdf->SetTextColor(5, 5, 5 );
	$pdf->Write(7,'    Libro. ');
	$pdf->SetFont('Times','B'.'U',12);
	$pdf->SetTextColor(5, 5, 5 );
	$pdf->Write(7,'   '.$data['librocm'].'   ');
	$pdf->SetFont('Times','',12);
	$pdf->SetTextColor(5, 5, 5 );
	$pdf->Write(7,'   se encuentra la fe de Primera Comunion que literalmente dice:   ');
    $pdf->Ln(10);
	...
	$pdf->SetFont('Times','',12);
	$pdf->Write(7,utf8_decode('Se le dio primera comunion a  '));
	$pdf->SetFont('Times','B'.'U',12).
