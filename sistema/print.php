...
$query = mysqli_query($conexion, "SELECT * FROM bautismos WHERE idbautismo='$id'");
...
while ($data = mysqli_fetch_assoc($query)) {
	...
	$pdf->Write(8,'En          ');
	$pdf->SetFont('Arial','B',12).
	$pdf->Write(8,$data['lugar']);
	$pdf->SetFont('Arial','',12).
	$pdf->Write(8,'           Municipio de          ');
	$pdf->SetFont('Arial','B',12).
	$pdf->Write(8,$data['municipio']);
	$pdf->SetFont('Arial','',12).
	$pdf->Write(8,'        Departamento         ');
	$pdf->SetFont('Arial','B',12).
	$pdf->Write(8,$data['departamento']);
	$pdf->SetFont('Arial','',12).
	$pdf->Ln();
	...
	$pdf->Write(8,'           y de           ');
	$pdf->SetFont('Arial','B',12).
	$pdf->Write(8,$data['padre']);
	$pdf->Ln();
	$pdf->SetFont('Arial','',12).
	$pdf->Write(8,'Fueron sus Padrinos      ');
	$pdf->SetFont('Arial','B',12).
	$pdf->Write(8,$data['madrina']);
	$pdf->SetFont('Arial','',12).
	$pdf->Write(8,'           y            ');
	$pdf->SetFont('Arial','B',12).
	$pdf->Write(8,$data['padrino']);
	$pdf->Ln();
	$pdf->SetFont('Arial','',12).
	$pdf->Write(8,'Ministro celebrante  ');
	$pdf->SetFont('Arial','B',12).
	$pdf->Write(8,$data['ministro']);
	$pdf->Ln();
	$pdf->SetFont('Arial','',12).
