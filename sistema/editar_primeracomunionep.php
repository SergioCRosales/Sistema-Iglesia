...
if (!empty($_POST)) {
  ...
  if (empty($_POST['idfeligres']) || empty($_POST['idmadrina']) || empty($_POST['idpadrino'])) {
    ...
    if (is_numeric($idfeligres) and $idfeligres != 0) {
      ...
      $query = mysqli_query($conexion, "SELECT * FROM comunionep where (idfeligres = '$idfeligres' AND idcomunion != $idcomunion)");
    ...
    if ($resul >= 1) {
      ...
      ?> <a target='_blank' href="constancia_comunionep.php?id=<?php echo $result['idcomunion']; ?>">
      ...
      $sql_update = mysqli_query($conexion, "UPDATE comunionep SET partida = '$partida', folio = '$folio', libro = '$libro', fecha= '$fecha', idfeligres = '$idfeligres', fechabautismo = '$fechabautismo', idmadre = '$idmadre', idpadre = '$idpadre', idmadrina = '$idmadrina', idpadrino = '$idpadrino', idministro = '$idministro', observaciones = '$observaciones' WHERE idcomunion = $idcomunion");
      ...
      if ($sql_update) {
        ...
        $result2 = 0;
        if (is_numeric($idfeligres) and $idfeligres != 0) {
            $query2 = mysqli_query($conexion, "SELECT * FROM comunionep where idfeligres = '$idfeligres'");
            $result2 = mysqli_fetch_array($query2);
        }

            ?> <a target='_blank' href="constancia_comunionep.php?id=<?php echo $result2['idcomunion']; ?>">
            <div align="center"> <img height="110" width="115" src="img/imprimir.jpg"><h4>Imprimir</h4> </div></a> <?php 
...
$sql = mysqli_query($conexion, "SELECT * FROM comunionep WHERE idcomunion = $idcomunion");
...
if ($result_sql == 0) {
  ...
  while ($data = mysqli_fetch_array($sql)) {
    ...
    $consultaactual= mysqli_query($conexion,"SELECT * FROM comunionep WHERE idcomunion='$idcomunion'");
...
?>
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h4 mb-0 text-gray-800">Editar datos de constancia de Primera Comunion (Otras Parroquias)</h1>
       
        <a href="lista_primeracomunionep.php" class="btn btn-primary">Ver listado comunion</a>
    </div>
            ...
            <div class="col-lg-6 m-auto">
              ...
              <form class="" action="" method="post">
                ...
                <div class="form-group">
                    ...
                    <input type="date" placeholder="Ingrese fecha para la primera comunionep" name="fechabautismo" id="fecha" class="form-control" value="<?php echo $fechabautismo; ?>"  required pattern="">
                </div>
